<template>
    <div>
        <van-checkbox-group v-model="result" @change="selectedProduct">
            <div v-for="(item,index) in list" :key="index">
                <van-swipe-cell :right-width="65" :on-close="onClose">
                    <div 
                        class="card bgc flex-align-items" 
                        @click="toggle(index)"
                        >
                        <van-checkbox :name="item.id" checked-color="#2DBBF1" ref="checkboxes"></van-checkbox>
                        <img src="http://img0.imgtn.bdimg.com/it/u=2486649772,2680843008&fm=26&gp=0.jpg" alt="" class="bgc-grey goodsimg">
        ...
        <div class="bottom bgc flex-align-items border-t">
            ...
            <div class="right flex-center">去结算<span v-show="result.length!=0">({{result.length}})</span></div>
        </div>
    </div>
</template>
...
export default {
    data(){
        return{
            list:[{id:1,price:1,num:2},{id:2,price:'3.03',num:1}],
            result: [],
            isall:false,
            sum:0
        }
