<template>
  <div>
    ...
    <div class="mar-b-10 main">
      <van-cell is-link center @click="go('/addresslist/friendbuy')" v-show="typenum==1||typenum==2">
        <div slot="title">
          <div>收货地址</div>
          <div>
      ...
      <template v-if="typenum==0">
        <van-cell is-link center @click="go(`/locationList/friendbuy/${goodid}`)">
          <template slot="title">
            <div>自取地点</div>
        ...
        <van-cell is-link center @click="chooseCalendar('/calendar/friendbuy/pre')">
          <template slot="title">
            <div>自取时间</div>
            <div>{{getdate}}</div>
          </template>
        </van-cell>
        <van-cell is-link center @click="go('/people/friendbuy')">
          <template slot="title">
            <div>自取人</div>
    ...
    <div class="mar-b-10">
      ...
      <van-cell
        ...
        @click="chooseCalendar('/calendar/expectdateTofriendbuy/pre')"
        :value="expectdate"
      >
        <template slot="title">
          <div>期望收到的日期</div>
...
export default {
  ...
  methods: {
    ...
    nextface() {
      if (this.isconsent) {
          if(JSON.parse(window.localStorage.getItem("userinfo")).users_id==this.$route.query.friendid){
              Dialog.alert({
                    message: '本人不能既是送礼人又是受礼人哦',
        ...
        if (this.typenum == 0) {
          ...
          var postData = this.$qs.stringify({
            ...
            order_type:1,
            giver_id: JSON.parse(window.localStorage.getItem("userinfo")).users_id
          });
        }
        if (this.typenum == 1) {
          if(this.getaddress == ""){
  ...
  activated() {
     if(!this.$route.meta.isBack || this.isFirstEnter){
       ...
       let friendbuySession = JSON.parse(window.sessionStorage.getItem("friendbuySession"));
        if (friendbuySession) {
          this.getlocation = friendbuySession.getlocation;
          this.getdate = friendbuySession.getdate;
          this.expectdate = friendbuySession.expectdate;
          this.timetext = friendbuySession.gettime;
          this.people = friendbuySession.getpeople;
          this.getaddress = friendbuySession.getaddress;
        }
     }
